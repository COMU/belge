...
<xsl:template match="itemizedlist/listitem">
  ...
  <li>
    ...
    <xsl:if test="local-name(child::*) = 'para'"><p/></xsl:if>
  </li>
</xsl:template>
  ...
  <div class="{name(.)}">
    ...
    <xsl:choose>
      <xsl:when test="$callout.list.table != 0">
        <table border="0" summary="Callout list" cellspacing="0" cellpadding="3" width="100%">
          <xsl:apply-templates/>
        </table>
      </xsl:when>
      <xsl:otherwise>
...
<xsl:template match="callout">
  <xsl:choose>
    <xsl:when test="$callout.list.table != 0">
      <tr>
        <td width="5%" valign="top" align="left" class="coimage">
          <xsl:call-template name="anchor"/>
