  ...
  <xsl:if test="string-length(&uppercase;) > $letternum">
    ...
    <xsl:if test="count(key('letter', $letter)[@scope=$target]) > 0">
      ...
      <div class="indexdiv">
        ...
        <dl>
          <xsl:apply-templates select="key('letter', $letter)[@scope=$target][ @id or count(.|key('primary', &primary;)[1]) = 1]"
                               mode="index-primary">
            <xsl:with-param name="is.first" select="1"/>
            <xsl:with-param name="target" select="$target"/>
            <xsl:sort select="translate(&primary;,&allcases;,&sortcases;)"/>
...
<xsl:template match="indexterm" mode="index-primary">
  ...
  <!--xsl:message>
      <xsl:value-of select="count($refs)"/>; <xsl:value-of select="count($refs.s)"/>;<xsl:value-of select="primary"/>
    </xsl:message-->
  <dt>
